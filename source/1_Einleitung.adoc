...
Der erste Film, den ich im Kino gesehen habe, war _Wallace & Gromit_. Dieser war nicht nur animiert, sondern setzte den "Claymationfilm" in Perfektion um. _Claymation_ ist ein Porte-Manteau Wort, aus den Englischen Wörtern "clay", zu Deutsch "Lehm" und "animation". Claymation beschreibt eine Methode der Filmanimation, bei welcher man mit modelierbaren Tonfiguren arbeitet und mit der Stop-Motion-Technik fotografiert. _Stop-Motion_ ist eine Filmtechnik, bei der eine Illusion von Bewegung erzeugt wird, indem man einzelne Standbilder aneinanderreiht.
